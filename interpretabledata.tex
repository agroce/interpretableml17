...
\title{User-Readable Guidance for\\Causally-Directed Testing}
...
\begin{abstract}
  Automated software testing frequently makes use of machine learning
  (either narrowly or broadly conceived) in the generation of tests.
  However, most such approaches only produce an ``end product'' of a
  set of tests, without information as to the derivation of thoses
  tests.  We propose a method that derives guidance for future test
  generation from a set of past tests (e.g. by a simple Markov-chain
  method), where those tests are decomposed in order to make the
  patterns that guide testing easy for users to understand.
\end{abstract}
